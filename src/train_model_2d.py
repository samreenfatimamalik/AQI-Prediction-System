...
# Trend: is pollution rising or falling over the week? (not just one lag point)
X_train["pm25_trend"] = X_train["pm25_lag_1"] - X_train["pm25_lag_7"]
X_test["pm25_trend"] = X_test["pm25_lag_1"] - X_test["pm25_lag_7"]

# Rolling max: worst day in the last week is a stronger spike-warning than the average
X_train["pm25_rolling_max_7"] = X_train[["pm25_lag_1", "pm25_lag_3", "pm25_lag_7", "pm25_rolling_7"]].max(axis=1)
X_test["pm25_rolling_max_7"] = X_test[["pm25_lag_1", "pm25_lag_3", "pm25_lag_7", "pm25_rolling_7"]].max(axis=1)
...
lr_mae, lr_rmse, lr_r2 = evaluate("Linear Regression (2d)", y_test, lr_preds)
...
rf_mae, rf_rmse, rf_r2 = evaluate("Random Forest tuned (2d)", y_test, rf_preds)
...
hgb_mae, hgb_rmse, hgb_r2 = evaluate("HistGradientBoosting tuned (2d)", y_test, hgb_preds)
...
joblib.dump(best_model, "models/model_2d.pkl")

# Register model in Hopsworks Model Registry
mr = project.get_model_registry()

model_2d = mr.python.create_model(
    name="aqi_model_2d",
    metrics=best_metrics,
    description=f"{best_model_desc} model predicting 2-day-ahead PM2.5 for 5 Pakistani cities"
)
model_2d.save("models/model_2d.pkl")
